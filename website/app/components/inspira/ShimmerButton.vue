...
withDefaults(defineProps<ShimmerButtonProps>(), {
  ...
  shimmerDuration: "3s",
  borderRadius: "100px",
  background: "rgb(6, 72, 60)",
});
</script>

<template>
  <button
    class="group relative z-0 flex transform-gpu cursor-pointer items-center justify-center overflow-hidden [border-radius:var(--radius)] border border-white/10 px-6 py-3 whitespace-nowrap text-white transition-transform duration-300 ease-in-out [background:var(--bg)] active:translate-y-px"
    :class="[$props.class]"
    :style="{
      '--spread': '90deg',
